...
import khoabd.utils.MyApplicationContanst.ShoppingViewServletConstant;
...
public class ShoppingViewServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = DBHelper.getUrl(this.getServletContext(),
                ShoppingViewServletConstant.SHOPPING_JSP_PAGE.value);
        try {
            ProductDAO dao = new ProductDAO();
            dao.loadAllProductToView();
            List<ProductDTO> result = dao.getListProduct();
            request.setAttribute("LIST_PRODUCT", result);
            
        } catch (SQLException ex) {
            log("Error at ShoppingViewServlet _SQL: " + ex.getMessage());
        } catch (NamingException ex) {
            log("Error at ShoppingViewServlet _Naming: " + ex.getMessage());
        } finally {
